layui.config({
    ...
    $(".usersAdd_btn").click(function () {
        var index = layui.layer.open({
            title: "添加货品",
            type: 2,
            content: "allAddDepot.html",
            success: function (layero, index) {
                setTimeout(function () {
                    layui.layer.tips('点击此处返回货品库存列表', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
                    });
                }, 500)
            }
    ...
    function delDepot(id){
        layer.confirm('确认删除吗？', function (confirmIndex) {
            ...
            $api.allDeleteDepot(req,function (data) {
    ...
    function editDepot(id){
        var index = layui.layer.open({
            title:"管理货品",
            type:2,
            content:"allEditDepot.html?id="+id,
            success: function (layero,index) {
                setTimeout(function () {
                    layui.layer.tips('点击此处返回货品库存列表', '.layui-layer-swtwin .layui-layer-close', {
                        tips: 3
                    });
                }, 500)
            }
