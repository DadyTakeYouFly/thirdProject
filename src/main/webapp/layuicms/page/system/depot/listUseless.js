layui.config({
    ...
    $(".usersAdd_btn").click(function () {
        var index = layui.layer.open({
            title: "添加货品",
            type: 2,
            content: "listAddUseless.html",
            success: function (layero, index) {
                setTimeout(function () {
                    layui.layer.tips('点击此处返回库存报废列表', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
                    });
                }, 500)
            }
    ...
    function delDepot(id){
        layer.confirm('确认删除吗？', function (confirmIndex) {
            ...
            $api.listDeleteUseless(req,function (data) {
    ...
    function inspectDepot(id) {
        var index = layui.layer.open({
            title: "审核报废单",
            type: 2,
            content: "listInspectUseless.html?id=" + id,
            success: function (layero, index) {
                setTimeout(function () {
                    layui.layer.tips('点击此处返回库存报废列表', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
    ...
    function editDepot(id){
        var index = layui.layer.open({
            title:"修改报废单",
            type:2,
            content:"listEditUseless.html?id="+id,
            success: function (layero,index) {
                setTimeout(function () {
                    layui.layer.tips('点击此处返回库存报废列表', '.layui-layer-swtwin .layui-layer-close', {
                        tips: 3
                    });
                }, 500)
            }
