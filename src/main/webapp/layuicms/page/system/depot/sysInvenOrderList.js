layui.config({
    ...
    function defineTable() {
        tableIns = table.render({
            elem: '#depot-data'
            , height: 415
            , url: $tool.getContext() + 'iolist/list.do' //数据接口
            , method: 'post'
            , page:true //开启分页
            , cols: [[ //表头
                //{title:'订单单号',width: '8%'},
                {type:'id',field: 'id', title: '库存订单编号',fixed: 'left', width: '10%'}
    ...
    function delDepot(id){
        layer.confirm('确认删除吗？', function (confirmIndex) {
            ...
            $api.ioDelete(req,function (data) {
    ...
    function editDepot(id){
        var index = layui.layer.open({
            title:"管理库存订单",
            type:2,
            content:"sysInvenOrderEdit.html?id="+id,
            success: function (layero,index) {
                setTimeout(function () {
                    layui.layer.tips('点击此处返回库存订单列表', '.layui-layer-swtwin .layui-layer-close', {
                        tips: 3
                    });
                }, 500)
            }
